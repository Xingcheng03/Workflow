...
// On 401 we refresh and retry exactly once. Dev/preview server only.
let crumbState = null;
...
const callQuoteSummary = (symbol, state) => {
  ...
  const url = `https://query2.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(
    symbol
  )}?modules=${modules}&crumb=${encodeURIComponent(state.crumb)}`;
  return fetch(url, {
    headers: { 'User-Agent': YAHOO_UA, Cookie: state.cookie }
  });
};

const marketSummaryHandler = async (req, res, next) => {
